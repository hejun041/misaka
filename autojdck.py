...
async def init_chrome():  # 判断chrome是否存在，不存在则下载，仅针对windows
    if platform.system() == 'Windows':
        chrome_dir = os.path.join(os.environ['USERPROFILE'], 'AppData', 'Local',
                                  'pyppeteer', 'pyppeteer', 'local-chromium', '1181205', 'chrome-win32')
        chrome_exe = os.path.join(chrome_dir, 'chrome.exe')
        chmod_dir = os.path.join(os.environ['USERPROFILE'], 'AppData', 'Local', 'pyppeteer',
                                 'pyppeteer', 'local-chromium', '1181205', 'chrome-win32', 'chrome-win32')
        if os.path.exists(chrome_exe):
            return
        else:
            print('貌似第一次使用，未找到chrome，正在下载chrome浏览器....')

            chromeurl = 'http://npm.taobao.org/mirrors/chromium-browser-snapshots/Win_x64/1181205/chrome-win32.zip'  # 定义下载地址
        ...
        if os.path.isfile(chrome_path):
            ...
            # download_url = "https://storage.googleapis.com/chromium-browser-snapshots/Linux_x64/678025/chrome-linux.zip"
            download_url = "https://registry.npmmirror.com/-/binary/chromium-browser-snapshots/Mac_Arm/1108028/chrome-mac.zip"
            if not os.path.exists(download_path):  # 如果没有路径就创建路径
                os.makedirs(download_path, exist_ok=True)  # 创建下载路径
            target_file = os.path.join(
                download_path, 'chrome-linux.zip')  # 定义下载文件路径跟文件名
        ...
        async with aiohttp.ClientSession() as session:                              # 异步操作命令
            ...
            async with session.get(url, headers=headers) as response:  # 获取变量请求
                ...
                if rjson['code'] == 200:  # 如果返回code200,根据青龙api文档
                    jd_cookie_data = [env for env in rjson['data'] if env.get(
                        'name') == 'JD_COOKIE']  # 获取全部jd的变量
                    global notess  # 把备注设置为全部变量
                    notess = [env['remarks'] for env in rjson['data'] if env.get('name') == 'JD_COOKIE' and env.get(
                        'status') == 0]  # 找到所有name为JD_COOKIE，status为0的字典列表，然后把remarks的值放进notess
        ...
        try:
            choice = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(
                    None, input, "需要进行短信验证，回1进行验证，回2不验证：  "),
                timeout=120
            )
...
async def validate_logon(usernum, passwd, notes):  # 登录操作
    ...
    browser = await launch({
        'executablePath': '/snap/bin/chromium',  # 指定 Chromium 的可执行文件路径
    ...
    # 访问京东登录页面
    await page.goto('https://plogin.m.jd.com/login/login?appid=300&returnurl=https%3A%2F%2Fm.jd.com%2F&source=wq_passport')
...
async def typeuser(page, usernum, passwd):  # 输入账户密码
    ...
    # 输入用户名，模拟键盘输入延迟
    await page.type('#username', usernum, {'delay': random.randint(60, 121)})
    # 输入密码，模拟键盘输入延迟
...
async def SubmitCK(page, notes):  # 提交ck
    ...
    print('{} 登录成功 pt_key={};pt_pin={};'.format(
        notes, pt_key, pt_pin))    # 打印 pt_key 和 pt_pin 值
    with open('jdck.log', 'a+', encoding='utf-8') as file:  # 打开文件
        content = '{}   {}   pt_key={};pt_pin={};\n'.format(datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S'), notes, pt_key, pt_pin)   # 构造要写入文件的字符串
...
async def duanxin(page):  # 短信验证函数
    ...
    await page.waitForXPath('//*[@id="app"]/div/div[2]/div[2]/span/a')
    await page.waitFor(random.randint(1, 3) * 1000)  # 随机等待1-3秒
    # 选择元素
    elements = await page.xpath('//*[@id="app"]/div/div[2]/div[2]/span/a')
    await elements[0].click()  # 点击元素
    # 等获取验证码元素
    await page.waitForXPath('//*[@id="app"]/div/div[2]/div[2]/button')
    await page.waitFor(random.randint(1, 3) * 1000)  # 随机等待1-3秒
    # 选择元素
    elements = await page.xpath('//*[@id="app"]/div/div[2]/div[2]/button')
    await elements[0].click()  # 点击元素
...
async def get_distance():  # 图形处理函数
    ...
    res = cv2.matchTemplate(
        img, template, cv2.TM_CCOEFF_NORMED)  # 使用模板匹配寻找最佳匹配位置
...
async def init_proxy_server():  # 初始化代理
    if proxy_server:  # 如果有配置代理
        # 设置浏览器参数加了个代理参数
        argszhi = '--no-sandbox', '--disable-setuid-sandbox', f'--proxy-server={proxy_server[0]}'
        return argszhi
    else:
        argszhi = '--no-sandbox', '--disable-setuid-sandbox', '–disable-infobars'
        return argszhi
...
async def main():  # 打开并读取配置文件，主程序
    ...
    os.remove('template.png') if os.path.exists(
        'template.png') else None  # 删除缓存照片
    await print_message('完成全部登录')
    await asyncio.sleep(10)  # 等待10秒，等待

asyncio.get_event_loop().run_until_complete(
    main())  # 使用异步I/O循环运行main()函数，启动整个自动登录和滑块验证流程。
